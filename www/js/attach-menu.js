...
function _openNativeAudioList(){
  ...
  function renderAudio(items){
    ...
    if(!items.length){
      list.innerHTML='<div style="text-align:center;padding:32px 20px"><p style="color:#6b7280;font-size:13px;margin:0 0 16px">No audio files found</p>'+
        '<button id="_audPickFb" style="background:linear-gradient(135deg,#7c3aed,#a855f7);color:#fff;border:none;border-radius:14px;padding:10px 20px;font-size:14px;font-weight:600;cursor:pointer">🎵 Browse audio files</button></div>';
      var fb=list.querySelector('#_audPickFb');
      if(fb) fb.onclick=function(){ ov.remove(); _audioFilePicker(); };
      return;
    }
...
// ═══ DOCUMENTS — WhatsApp-style top-level Files sheet ════════════════════════
function _openNativeDocList(){
  // Top-level picker: Browse documents | Browse audio (mirrors WhatsApp Files sheet)
  var ov=_mkSheet('Files',
    '<div style="padding:4px 0 8px">'+
    '<div class="_mRow" id="_amBrowseDoc" style="cursor:pointer">'+
      '<div class="_mIcon" style="background:linear-gradient(135deg,#22c55e,#16a34a)">'+
        '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="1.8"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>'+
      '</div>'+
      '<div style="flex:1;min-width:0"><div class="_mName">Browse documents</div><div class="_mMeta">PDF, Word, Excel, PowerPoint, CSV…</div></div>'+
    ...
    '</div>'+
    '<div class="_mRow" id="_amBrowseAudio" style="cursor:pointer">'+
      '<div class="_mIcon" style="background:linear-gradient(135deg,#7c3aed,#a855f7)">'+
        '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="1.8"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>'+
      '</div>'+
      '<div style="flex:1;min-width:0"><div class="_mName">Browse audio</div><div class="_mMeta">MP3, OGG, WAV, M4A, AAC…</div></div>'+
      '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6b7280" stroke-width="2" style="flex-shrink:0"><polyline points="9 18 15 12 9 6"/></svg>'+
    '</div>'+
    '</div>'
  );
  ov.querySelector('#_amBrowseDoc').onclick=function(){ ov.remove(); _openDocumentList(); };
  ov.querySelector('#_amBrowseAudio').onclick=function(){ ov.remove(); _openNativeAudioList(); };
}
// Internal: show CapacitorMediaStore doc list, or fall back to file picker
function _openDocumentList(){
  var MS=window.Capacitor&&window.Capacitor.Plugins&&window.Capacitor.Plugins.CapacitorMediaStore;
  if(!MS){ _docFilePicker(); return; }
  var inner=_mkSheet('Documents',
  ...
  function renderDocs(items){
    var list=inner.querySelector('#_dList');
    if(!items.length){
      list.innerHTML='<div style="text-align:center;padding:32px 20px"><p style="color:#6b7280;font-size:13px;margin:0 0 16px">No documents found</p>'+
        '<button id="_docPickFb" style="background:linear-gradient(135deg,#22c55e,#16a34a);color:#fff;border:none;border-radius:14px;padding:10px 20px;font-size:14px;font-weight:600;cursor:pointer">📁 Open file manager</button></div>';
      var fb=list.querySelector('#_docPickFb');
      if(fb) fb.onclick=function(){ inner.remove(); _docFilePicker(); };
      return;
    }
    ...
    inner.querySelectorAll('._mRow').forEach(function(row){
      row.onclick=function(){ var item=items[parseInt(row.getAttribute('data-idx'))]; inner.remove(); if(_attachCB&&item){var cb=_attachCB;_attachCB=null;cb({_isMediaUri:true,uri:item.uri,name:item.name,size:item.size,mediaType:'document',text:item.name});} };
    });
  }
  MS.getMediasByType({mediaType:'document',includeExternal:true}).then(function(r){
    ...
    renderDocs(allDocs);
  }).catch(function(){ inner.remove(); _docFilePicker(); });
  var si=inner.querySelector('#_dSrch');
...
function _docFilePicker(){ var inp=_tmpInput('.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,application/pdf,application/msword,application/vnd.ms-excel,application/vnd.ms-powerpoint,text/plain,text/csv'); inp.onchange=function(){if(inp.files&&inp.files[0]&&_attachCB){var cb=_attachCB;_attachCB=null;cb(inp.files[0]);}inp.value='';};inp.click();}
